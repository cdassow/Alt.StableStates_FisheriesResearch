...
ggsave('alt.fig1_revision.pdf', plot = alt.fig1, width = 7, height = 5)
...
alt.fig2
ggsave('alt.fig2_revision.pdf', plot = alt.fig2, width = 7, height = 5)
...
alt.fig3
ggsave('alt.fig3_revision.pdf', plot = alt.fig3, width = 7, height = 5)
...
sim=ode(y=y0,times=tstep,func=simBiggsR.aw,parms=p)
...
simS=ode(y=y0,times=tstep,func=simBiggsR.aw,parms=p)
...
simH=ode(y=y0,times=tstep,func=simBiggsR.aw,parms=p)
...
simB=ode(y=y0,times=tstep,func=simBiggsR.aw,parms=p)

#plotting

#do nothing
panA=as.data.frame(rbind(cbind(sim[,c(1,2)],rep('Preferred Species',nrow(sim))),cbind(sim[,c(1,3)],rep('Competitor Species',nrow(sim)))))
...
#harvest sp2
panB=as.data.frame(rbind(cbind(simH[,c(1,2)],rep('Preferred Species',nrow(simH))),cbind(simH[,c(1,3)],rep('Competitor Species',nrow(simH)))))
...
#stock sp1
panC=as.data.frame(rbind(cbind(simS[,c(1,2)],rep('Preferred Species',nrow(simS))),cbind(simS[,c(1,3)],rep('Competitor Species',nrow(simS)))))
...
#stock and harvest
panD=as.data.frame(rbind(cbind(simB[,c(1,2)],rep('Preferred Species',nrow(simB))),cbind(simB[,c(1,3)],rep('Competitor Species',nrow(simB)))))
...
alt.fig4
ggsave('alt.fig4_revision.pdf', plot = alt.fig4, width = 7, height = 5)
